...
- Clerk live keys on `*.vercel.app` cannot use a `clerk.<vercel-app-host>` Frontend API host (TLS/CNAME impossible under `*.vercel.app`). `@clerk/nextjs@6.39.x` has no `frontendApiProxy` — keep the `/__clerk` proxy in `next_app/src/middleware.ts` + `src/lib/clerkFrontendApiProxy.ts`, and derive `proxyUrl` for `pk_live_` (or `NEXT_PUBLIC_CLERK_PROXY_URL`). After deploy, click **Verify** on the Clerk Dashboard domain proxy.
- `next_app/scripts/next-build.mjs` must **not** substitute the `clerk.joinup.local` placeholder when `VERCEL=1` and `VERCEL_ENV=production` — that ships broken auth. Fail the build if `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` / `CLERK_SECRET_KEY` are missing on Production.
