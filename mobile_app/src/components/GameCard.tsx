        ...
        >
            ...
            <View className="relative h-40">
                ...
                <View className="absolute top-4 left-4 bg-black/50 px-3 py-1.5 rounded-full border border-white/20">
                    <Text className="text-white text-xs font-bold">{game.sport || "ספורט"}</Text>
                </View>
                {isJoined && (
                    <View className="absolute top-4 right-4 bg-green-500 px-3 py-1.5 rounded-full shadow-sm">
                        <Text className="text-white text-xs font-bold">הצטרפת ✅</Text>
                    </View>
                )}
            </View>
            ...
            <View className={`p-5 ${isJoined ? 'bg-green-50/30' : ''}`}>
                <View className="flex-row-reverse justify-between items-center mb-3">
                    <View className="flex-row-reverse items-center bg-blue-50 px-3 py-1.5 rounded-xl">
                        <Ionicons name="time-outline" size={14} color="#2563eb" />
                        <Text className="mr-1.5 text-blue-700 font-bold text-xs">
                            {displayDate} • {game.time}
                        </Text>
                    </View>
                    <View className="flex-row-reverse items-center">
                        {game.price ? (
                            <View className="bg-green-50 px-2 py-1 rounded-lg border border-green-100 ml-2">
                ...
                <Text className="text-xl font-black text-gray-900 mb-1 text-right" numberOfLines={1}>
                    {game.title || game.fieldName || "משחק כדורגל"}
                </Text>
                <View className="flex-row-reverse items-center mb-4">
                    <Ionicons name="location-outline" size={12} color="#9ca3af" />
                    <Text className="mr-1 text-gray-400 text-xs flex-1 text-right" numberOfLines={1}>
                        {game.fieldLocation}
                    </Text>
                </View>
